...
if (!db.customExerciseNames) db.customExerciseNames = {};

// Reaplica nomes de exercícios renomeados manualmente (persistidos), sobrescrevendo os nomes padrão do código
Object.entries(db.customExerciseNames).forEach(([wKey, exMap]) => {
    if (!DATA[wKey]) return;
    Object.entries(exMap).forEach(([idx, name]) => {
        if (DATA[wKey].ex[idx]) DATA[wKey].ex[idx][0] = name;
    });
});
    ...
    modal.innerHTML = `
        <div class="modal-box">
            ...
            <div style="display:flex; gap:8px; margin-top:15px;">
                ...
                <button class="primary" style="flex:1; padding:10px; background:linear-gradient(135deg, #8a2be2, #a855f7);" onclick="this.parentElement.parentElement.parentElement.remove()">Fechar</button>
            </div>
        </div>
    `;
    document.body.appendChild(modal);
...
function saveEditedExercises(workoutKey) {
    const workout = DATA[workoutKey];
    db.customExerciseNames[workoutKey] = db.customExerciseNames[workoutKey] || {};
    workout.ex.forEach((ex, idx) => {
        const inputVal = document.getElementById(`editEx-${workoutKey}-${idx}`)?.value.trim();
        if (inputVal) {
            ex[0] = inputVal;
            db.customExerciseNames[workoutKey][idx] = inputVal;
        }
    });
    save();
